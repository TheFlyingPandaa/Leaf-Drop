...
public:
	...
	HRESULT Init(const UINT & width, const UINT & height, const UINT & arraySize = 1, const UINT & maxMips = 1, const DXGI_FORMAT & format = DXGI_FORMAT_R8G8B8A8_UNORM);
	
	void Begin(ID3D12GraphicsCommandList * commandList) const;
	void End(ID3D12GraphicsCommandList * commandList) const;
	void CopySubresource(ID3D12Resource * resource, const UINT & dstIndex, ID3D12GraphicsCommandList * commandList) const;
